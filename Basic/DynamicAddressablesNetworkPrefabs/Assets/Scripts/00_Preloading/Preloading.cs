...
namespace Game.Preloading
{
    ...
    public sealed class Preloading : MonoBehaviour
    {
        ...
        async Task PreloadDynamicPlayerPrefab()
        {
            ...
            opHandle = Addressables.LoadAssetAsync<GameObject>(m_DynamicPrefabReference);
            var prefab = await opHandle.Task;
        ...
        void OnDestroy()
        {
            // asset can now be unloaded from memory -- doing so here allows for subsequent hosting and joining sessions
            // while playing inside this scene
